...
def generate_alu(constraint_path: str, output_dir: str = "src") -> None:
    """
    Read the JSON constraints at `constraint_path` and, using Jinja templates
    in ./templates, emit a set of .sv files into `./src`.

    Raises:
      FileNotFoundError   if `constraint_path` does not exist.
      ValueError          if no operations are selected in the JSON.
    """

    # === Check if constraints file exists === #
    if not os.path.exists(constraint_path):
        raise FileNotFoundError(f"Constraints file '{constraint_path}' does not exist.")
    ...
    user_ops = constraints.get("user_opcodes", [])
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python generate.py <path/to/constraints.json>")
        sys.exit(1)

    constraint_path = sys.argv[1]
    try:
        generate_alu(constraint_path, output_dir="src")
